...
class UserRegister(Resource):    
    ...
    def post(self, email, password, device_id):
        ...
        conn = open_connection()
        with conn.cursor() as cursor:
            cursor.execute(f'INSERT INTO table_user (col_email, col_password, col_deviceid) VALUES({email}, {password}, {device_id}) WHERE NOT EXISTS (SELECT * FROM table_user WHERE code={email})') 
